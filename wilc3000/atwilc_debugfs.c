/*
* Atmel WiFi chipset driver test tools - atwilc-debug
* Copyright (c) 2012 Atmel Corp.
* Author: SSW <sswd@atwilcsemic.com> 
*
...
#include "atwilc_queue.h"
#include "atwilc_wlan_if.h"


static struct dentry *atwilc_dir;
...
atomic_t REGION = ATOMIC_INIT(INIT_DBG | GENERIC_DBG | CFG80211_DBG | PWRDEV_DBG);
...
#define DUMP_QUEUE_MAX (16*1024) // (8*1024)
DECLARE_WAIT_QUEUE_HEAD(atwilc_msgdump_waitqueue);


/*
--------------------------------------------------------------------------------
*/
static int atwilc_debug_level_open(struct inode *inode, struct file *file)
{
	/*DEBUG_LEVEL = *((unsigned int *)inode->i_private);*/
	printk("atwilc_debug_level_open: current debug-level=%x\n", atomic_read(&DEBUG_LEVEL));
	return 0;
}

static int atwilc_debug_region_open(struct inode *inode, struct file *file)
{
	/*REGION = *((unsigned int *)inode->i_private);*/
	printk("atwilc_debug_region_open: current debug-region=%x\n", atomic_read(&REGION));
	return 0;
}

static int atwilc_debug_dump_open(struct inode *inode, struct file *file)
{
	printk("atwilc_debug_dump_open\n");
	return 0;
}

static ssize_t atwilc_debug_level_read(struct file *filp, char *buf, size_t count, loff_t *ppos)
{
...
static ssize_t atwilc_debug_level_write(struct file *filp, const char *buf, size_t count, loff_t *ppos)
{
	unsigned int flag = 0;
...
static ssize_t atwilc_debug_region_read(struct file *filp, char *buf, size_t count, loff_t *ppos)
{
...
static ssize_t atwilc_debug_region_write(struct file *filp, const char *buf, size_t count, loff_t *ppos)
{
	uint32_t flag = 0;
...
static unsigned int kmsgdump_poll(struct file *file, poll_table *wait)
{
	...
	poll_wait(file, &atwilc_msgdump_waitqueue, wait);
	if(queue_data_size(&g_dumpqueue))
		return POLLIN | POLLRDNORM;
	return 0;
}
