...
static atwilc_mac_cfg_t g_mac;

static atwilc_cfg_byte_t g_cfg_byte[] = {
	{WID_BSS_TYPE, 0},
...
static atwilc_cfg_hword_t g_cfg_hword[] = {
...
static atwilc_cfg_word_t g_cfg_word[] = {
	{WID_FAILED_COUNT, 0},
	{WID_RETRY_COUNT, 0},
...
static atwilc_cfg_str_t g_cfg_str[] = {
...
static int atwilc_wlan_cfg_set_byte(uint8_t *frame, uint32_t offset, uint16_t id, uint8_t val8)
{
	uint8_t *buf;
...
static int atwilc_wlan_cfg_set_hword(uint8_t *frame, uint32_t offset, uint16_t id, uint16_t val16)
{
	uint8_t *buf;
...
static int atwilc_wlan_cfg_set_word(uint8_t *frame, uint32_t offset, uint16_t id, uint32_t val32)
{
	uint8_t *buf;
...
static int atwilc_wlan_cfg_set_str(uint8_t *frame, uint32_t offset, uint16_t id, uint8_t *str, uint32_t size)
{
	uint8_t *buf;
...
static int atwilc_wlan_cfg_set_bin(uint8_t *frame, uint32_t offset, uint16_t id, uint8_t *b, uint32_t size)
{
	uint8_t *buf;
	uint32_t i;
	uint8_t checksum = 0;
...
static void atwilc_wlan_parse_response_frame(uint8_t *info, int size)
{
...
static int atwilc_wlan_parse_info_frame(uint8_t *info, int size)
{
	atwilc_mac_cfg_t *pd = (atwilc_mac_cfg_t *)&g_mac;
	uint32_t wid, len;
	int type = ATWILC_CFG_RSP_STATUS;
...
#if 0
static int atwilc_wlan_parse_network_frame(uint8_t *info, int size)
{
	atwilc_mac_cfg_t *priv = (atwilc_mac_cfg_t *)&g_mac;
	uint32_t wid, len;
...
static int atwilc_wlan_cfg_set_wid(uint8_t *frame, uint32_t offset, uint16_t id, uint8_t *buf, int size)
{
	...
	if (type == 0) {					/* byte command */
		if (size >= 1)
			ret = atwilc_wlan_cfg_set_byte(frame, offset, id, *buf);		
	} else if (type == 1) {			/* half word command */
		if (size >= 2)
			ret = atwilc_wlan_cfg_set_hword(frame, offset, id, *((uint16_t *)buf));
	} else if (type == 2) {			/* word command */
		if (size >= 4)
			ret = atwilc_wlan_cfg_set_word(frame, offset, id, *((uint32_t *)buf));
	} else if (type == 3) {			/* string command */
		ret = atwilc_wlan_cfg_set_str(frame, offset, id, buf, size);
	} else if (type == 4) {			/* binary command */
		ret = atwilc_wlan_cfg_set_bin(frame, offset, id, buf, size);
	} else {
...
static int atwilc_wlan_cfg_get_wid(uint8_t *frame, uint32_t offset, uint16_t id)
{
	uint8_t *buf;
...
static int atwilc_wlan_cfg_get_wid_value(uint16_t wid, uint8_t *buffer, uint32_t buffer_size)
{
...
static int atwilc_wlan_cfg_indicate_rx(uint8_t *frame, int size, atwilc_cfg_rsp_t *rsp)
{
	int ret = 1;
	uint8_t msg_type;
	uint8_t msg_id;
	uint16_t msg_len;
	#ifdef ATWILC_FULLY_HOSTING_AP
	ATL_Uint32* ptru32Frame;
	ATL_Bool bStatus = frame[2];
	...
	**/
	...
	switch(msg_type) {
	case 'R':
		atwilc_wlan_parse_response_frame(frame, size);
		rsp->type = ATWILC_CFG_RSP;
		rsp->seq_no = msg_id;
		break;
	case 'I':
		rsp->type = atwilc_wlan_parse_info_frame(frame, size);
		rsp->seq_no = msg_id;
...
#if 1
	case 'N':
		//ATL_PRINTF("[cfg]Network Message Received\n");
...
/*bug3819:*/
	...
	case'A':
		...
		ATWILC_mgm_HOSTAPD_ACK(ptru32Frame,bStatus);
...
static int atwilc_wlan_cfg_init(atwilc_debug_func func)
{
	memset((void *)&g_mac, 0, sizeof(atwilc_mac_cfg_t));
	g_mac.dPrint = func;
	return 1;
}

atwilc_cfg_func_t mac_cfg = {
	atwilc_wlan_cfg_set_wid,
	atwilc_wlan_cfg_get_wid,
	atwilc_wlan_cfg_get_wid_value,
	atwilc_wlan_cfg_indicate_rx,
	atwilc_wlan_cfg_init,
};
