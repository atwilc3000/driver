...
static const ATL_Uint8  g_bmap[8] = {1, 2, 4, 8, 16, 32, 64, 128}; /* Bit map */
...
ATL_Uint8   g_snap_header[SNAP_HDR_ID_LEN] = {0xAA, 0xAA, 0x03, 0x00, 0x00, 0x00};

/* Power management related globals */
ATL_Uint8       g_num_sta_ps              = 0; /* Num of STA in PS */
ATL_Uint32      g_num_mc_bc_pkt           = 0; /* Num of BC/MC pkts in PSQ     */
ATL_Uint32		 g_num_ps_pkt     		   = 0; /* Num of UC/BC/MC pkts in PSQ  */
ATL_Uint16      g_num_mc_bc_qd_pkt    	   = 0; /* Num of BC/MC pkts in Hw  	*/
list_buff_t  g_mc_q;
//ATL_Uint8       g_vbmap[VBMAP_SIZE]       = {0,};
//ATL_Uint8       g_tim_element_index       = 0;
//ATL_Uint16      g_tim_element_trailer_len = 0;
q_head_t     g_ps_pending_q            = {0,};
ATL_Uint16      g_aging_thresh_in_sec     = AGING_THR_IN_SEC;
ATWILC_ieee80211_sta *g_max_ps_ae = NULL;      /* Association entry of STA, with max num of PS pkts */


void* find_entry(ATL_Uint8* key);
void add_entry(void* entry, ATL_Uint8* key);
void delete_entry(ATL_Uint8* key);
void frmw_to_linux(uint8_t *buff, uint32_t size);
void ap_enabled_rx_data( ATL_Uint8 *msg);
REQUEUE_STATUS_T requeue_ps_packet(ATWILC_ieee80211_sta* ae, list_buff_t *qh,
                                   ATL_Bool ps_q_legacy, ATL_Bool eosp);
ATL_Bool buffer_tx_packet(ATL_Uint8 *entry, ATL_Uint8 *da, ATL_Uint8 priority,
                         struct tx_complete_data *tx_dscr);
void handle_ps_tx_comp_ap(struct tx_complete_data *tx_dscr );
void check_and_reset_tim_bit(ATL_Uint16 asoc_id);
ATL_Bool handle_ps_poll(wlan_rx_t    *wlan_rx );


/* This function returns whether the Multicast bit is set in the given beacon*/
/* frame                                                                     */
inline ATL_Bool get_mc_bit_bcn(void)
{
    ATL_Bool ret_value;
    ret_value = (ATL_Bool)(strBeaconInfo.u8beacon_frame[strBeaconInfo.u8tim_element_index + BMAP_CTRL_OFFSET] &
            0x01);
    return (ret_value);
}
...
/*
*  @brief 			ATWILC_beacon_tx_complete
...
*  @version		1.0
*/	
static void ATWILC_beacon_tx_complete(void* priv, int status)
{
	 // Do nothing 
...
*  @version		1.0
*/	
static int ATWILC_beacon_xmit(const u8 *buf, size_t len)
{
	...
	//ATL_PRINTF("--IN beacon_tx: Sending beacon Pkt to tx queue--\n");
	atwilc_wlan_txq_add_mgmt_pkt(strBeacon_tx,strBeacon_tx->buff,strBeacon_tx->size,ATWILC_beacon_tx_complete);
									 ...
									 ATL_Uint32 u32TailLen, ATL_Uint8* pu8Tail)
{
	...
	ATL_Bool bIsBeaconSet = ATL_FALSE;
	ATL_Uint8 *pu8BeaconFrame;
	ATL_Uint16  u16size=0;
	atwilc_wlan_dev_t* strWlan = Get_wlan_context(&u16size); 

	if(u16size != sizeof(atwilc_wlan_dev_t))
		PRINT_ER("size of atwilc_wlan_dev_t in atwilc_wlan != it's size in atwilc_host_ap\n");


	//ATL_PRINTF("--IN host_add_beacon--\n");
	...
	if(strBeaconInfo.u8beacon_frame == NULL)
	{
		...
		strBeaconInfo.u8beacon_frame  = (ATL_Uint8 *)kmalloc(strBeaconInfo.u16beacon_len,GFP_ATOMIC);
		if(strBeaconInfo.u8beacon_frame  == NULL)
		{
	...
	ATL_memcpy(pu8BeaconFrame, pu8Head, u32HeadLen);
	...
	*(pu8BeaconFrame++) = 0;
	...
	ATL_memcpy(pu8BeaconFrame, pu8Tail, u32TailLen);
	pu8BeaconFrame+=u32TailLen;

	ATWILC_beacon_xmit(strBeaconInfo.u8beacon_frame,strBeaconInfo.u16beacon_len);
...
void process_tbtt_isr(void)
{
       ...
       ATL_Uint16 i           = 0;
	ATL_Uint8 u8OldTimLen = 0;
	ATL_Uint32 dtim_count  = 0;
	ATL_Uint16  u16size=0;
	atwilc_wlan_dev_t* strWlan = Get_wlan_context(&u16size); 
	...
	strWlan->hif_func.hif_read_reg(rMAC_DTIM_COUNT_ADDR, &dtim_count);
       ...
       //ATL_PRINTF("dtim_count = %d",dtim_count);
        ...
        if(dtim_count == 0)
        {
            ...
            /* set                                                           */
            if(ATL_TRUE == get_mc_bit_bcn())
                while(requeue_ps_packet(NULL, &g_mc_q, ATL_TRUE, ATL_FALSE)
                      == PKT_REQUEUED);
	...
	if(strBeaconInfo.u8beacon_frame != NULL)
	{
		...
		{
			ATL_Uint32 u32ShiftOffset = u8OldTimLen - strBeaconInfo.u8vbmap[LENGTH_OFFSET];
			ATL_Uint32 u32TrailerIndex = strBeaconInfo.u8tim_element_index+u8OldTimLen+2;
		...
		{
			ATL_Uint32 u32ShiftOffset = strBeaconInfo.u8vbmap[LENGTH_OFFSET] - u8OldTimLen;
			ATL_Uint32 u32TrailerIndex = strBeaconInfo.u8tim_element_index+u8OldTimLen+2;
...
inline void init_list_buffer(list_buff_t *lbuff, ATL_Uint32 lnk_byte_ofst)
{
    lbuff->head          = NULL;
    lbuff->tail          = NULL;
    lbuff->count         = 0;
...
inline void add_list_element(list_buff_t *lbuff, void *elem)
{
    ...
    if(lbuff->tail == NULL)
        ...
        NEXT_ELEMENT_ADDR(lbuff->tail, lbuff->lnk_byte_ofst) = (ATL_Uint32)elem;
                                    ...
                                    void *curr_el)
{
    ...
    {
        NEXT_ELEMENT_ADDR(prev_el, lbuff->lnk_byte_ofst) = (ATL_Uint32)next_el;
    }
...
/*!
 *  @fn		void ATWILC_AP_AddSta(u8 *mac, struct station_parameters *params)
...
{
	ATL_Uint8 au8Sa[6];
	ATL_Uint8 * pu8CurrData;
	ATWILC_ieee80211_sta* pstrAsocEntry = NULL;
	ATL_Uint32 i;
	ATL_Bool bIsPrevAsoc = ATL_FALSE;
	ATL_Uint8 ht_info_mod = 0;
	ATL_Uint16 u16CapInfo = 0;
	
	
	ATL_memcpy(au8Sa, mac, ETH_ALEN);

	PRINT_D(HOSTAPD_DBG,"Add station BSSID: %x, %x, %x,%x,%x,%x,\n",
			au8Sa[0], au8Sa[1], au8Sa[2], au8Sa[3], au8Sa[4], au8Sa[5]);

	pstrAsocEntry = (ATWILC_ieee80211_sta*)find_entry(au8Sa);


	pstrAsocEntry = (ATWILC_ieee80211_sta *)kmalloc(sizeof(ATWILC_ieee80211_sta),GFP_ATOMIC);	
	...
	/* Reset the AE Handle */
	ATL_memset(pstrAsocEntry, 0, sizeof(ATWILC_ieee80211_sta));
	...
	*/
	...
	pstrAsocEntry->ps_poll_rsp_qed  = ATL_FALSE;
	...
	{
		ATL_Uint8 min = (ae->op_rates.rates[i] & 0x7F);
		ATL_Uint8 mid = i;
		ATL_Uint8 tmp = 0;
	...
	{
		ATL_Bool found = ATL_FALSE;
		ATL_Uint8 mac_br = get_mac_basic_rate(i);
	...
	pstrAsocEntry->ht_cap.ht_supported = (params->ht_capa != ATL_NULL);
	PRINT_D(HOSTAPD_DBG,"HT supported: %d\n", pstrAsocEntry->ht_cap.ht_supported);
	if(pstrAsocEntry->ht_cap.ht_supported)
	{
	/*
	pu8CurrData += ATWILC_AP_ParseHTCap(pu8CurrData, &pstrAsocEntry->ht_hdl,
	...
	//{
		...
		/*if(get_11n_op_mode() == HT_MIXED_MODE)
		{
			...
			/*if((is_autoprot_enabled() == ATL_TRUE) &&
	...
	//pstrAsocEntry->tx_rate_mbps = get_user_rate((ATL_Uint8)
	...
	else
	{
		...
		if((pstrAsocEntry->state == ASOC) && (pstrAsocEntry->short_slot == 0))
		{
			...
			{
				if(g_short_slot_allowed == ATL_TRUE)
				{
					enable_short_slot();
	...
	/* UNSUP_RATE and not allow the station to associate with AP.        */
		...
		if((u16CapInfo & SHORTPREAMBLE) != SHORTPREAMBLE)
		{
			...
			if((pstrAsocEntry->state != ASOC) || (pstrAsocEntry->short_preamble == 1))
			{
				...
				g_short_preamble_enabled = ATL_FALSE;
		...
		else
		{
			...
			if((pstrAsocEntry->state == ASOC) && (pstrAsocEntry->short_preamble == 0))
			{
				...
				{
					g_short_preamble_enabled = ATL_TRUE;
...
/*!
 *  @fn		void ATWILC_AP_EditSta(u8 *mac, struct station_parameters *params)
...
inline void ATWILC_AP_EditSta(u8 *mac, struct station_parameters *params)
{
	ATL_Uint8 au8Sa[6];
	ATWILC_ieee80211_sta* pstrAsocEntry = NULL;
	ATL_Uint8 u8NumRatesOld = 0;
	ATL_Uint16 u16Mask, u16Set;
...
/*!
 *  @fn			inline void ATWILC_AP_RemoveSta(ATL_Uint8 * pu8StaInfo)
...
inline ATL_Bool is_snap_header_present(wlan_rx_t *strWlan_rx)
{
    ATL_Uint8 *data = NULL;
    ...
    if(ATL_memcmp(data, g_snap_header, SNAP_HDR_ID_LEN) != 0)
            return ATL_FALSE;
...
inline void set_frame_control(ATL_Uint8* header, ATL_Uint16 fc)
{
   header[0] = (ATL_Uint8)(fc & 0x00FF);
   header[1] = (ATL_Uint8)(fc >> 8);
}

inline ATL_Uint8 set_mac_hdr(ATL_Uint8 *mac_hdr)
{
...
inline ATL_Uint8 set_mac_hdr_prot(ATL_Uint8 *mac_hdr, ATL_Uint8 priority,
                               ATL_Uint8 service_class, ATL_Bool qos, ATL_Bool is_ht,
                               ATL_Uint8 is_amsdu)
{
	// TODO: implement 11n
#if 0
...
inline void set_from_ds(ATL_Uint8* header, ATL_Uint8 from_ds)
{
    header[1] &= 0xFD;
    header[1] |= (from_ds << 1);
}
...
/* ignored.                                                                  */
inline  void set_address1(ATL_Uint8* msa, ATL_Uint8* addr)
{
    ATL_memcpy(msa + 4, addr, ETH_ALEN);
}

/* This function sets the 'address2' field in the MAC header of the input    */
/* frame to the input MAC Address 'addr'. The 16 LSB bits of 'addr' are      */
/* ignored.                                                                  */
inline  void set_address2(ATL_Uint8* msa, ATL_Uint8* addr)
{
    ATL_memcpy(msa + 10, addr, ETH_ALEN);
}
...
/* frame to the input MAC Address 'addr'. The 16 LSB bits of 'addr' are      */
/* ignored.                                                                  */
inline  void set_address3(ATL_Uint8* msa, ATL_Uint8* addr)
{
    ATL_memcpy(msa + 16, addr, ETH_ALEN);
}


/* This function extracts the MAC Address in 'address1' field of the MAC     */
/* header and updates the MAC Address in the allocated 'addr' variable.      */
inline void get_address1(ATL_Uint8* msa, ATL_Uint8* addr)
{
    ATL_memcpy(addr, msa + 4, ETH_ALEN);
}

/* This function returns the pointer to the MAC Address in 'Address-1" field */
/* of the MAC header.                                                        */
inline ATL_Uint8 *get_address1_ptr(ATL_Uint8* msa)
{
    return (msa + 4);
}
...
/* header and updates the MAC Address in the allocated 'addr' variable.      */
inline void get_address2(ATL_Uint8* msa, ATL_Uint8* addr)
{
    ATL_memcpy(addr, msa + 10, ETH_ALEN);
}

/* This function extracts the MAC Address in 'address3' field of the MAC     */
/* header and updates the MAC Address in the allocated 'addr' variable.      */
inline void get_address3(ATL_Uint8* msa, ATL_Uint8* addr)
{
    ATL_memcpy(addr, msa + 16, ETH_ALEN);
}

/* This function returns the pointer to the MAC Address in 'Address-3" field */
/* of the MAC header.                                                        */
inline ATL_Uint8 *get_address3_ptr(ATL_Uint8* msa)
{
    return (msa + 16);
}
...
inline BASICTYPE_T get_type(ATL_Uint8* header)
{
...
/* Returns the value in the LSB of the returned value.                       */
inline TYPESUBTYPE_T get_sub_type(ATL_Uint8* header)
{
...
/* the input frame.                                                          */
/* Returns the value in the LSB of the returned value.                       */
inline ATL_Uint8 get_pwr_mgt(ATL_Uint8* header)
{
...
/* Returns ATL_TRUE if the input address has the group bit set.                 */
inline ATL_Bool is_group(ATL_Uint8* addr)
{
    if((addr[0] & BIT0) != 0)
        return ATL_TRUE;
...
inline ATL_Bool is_sub_type_null_prot(ATL_Uint16 frm_type)
{
...
/* the input frame.                                                          */
/* Returns the value in the LSB of the returned value.                       */
inline ATL_Uint8 get_protocol_version(ATL_Uint8* header)
{
...
/* returns ATL_TRUE, ATL_FALSE otherwise                                           */
inline ATL_Bool check_from_to_ds(wlan_rx_t *wlan_rx)
{
...
/* Returns ATL_Bool, ATL_TRUE if the two addresses are same and ATL_FALSE otherwise. */
inline ATL_Bool mac_addr_cmp(ATL_Uint8* m1, ATL_Uint8* m2)
{
    if(memcmp(m1, m2, 6) == 0)
        return ATL_TRUE;
...
inline ATL_Bool check_bssid_match(wlan_rx_t *strWlan_rx)
{

	linux_wlan_t *pd = g_linux_wlan;
	ATL_Uint8*  u8macaddr = NULL;  
...
{
    ATL_Uint8 frm_ds = strWlan_rx->u8frm_ds;
    ATL_Uint8 to_ds  = strWlan_rx->u8to_ds;
    if((to_ds == 0) && (frm_ds == 0))
    {
...
inline ATL_Bool is_qos_bit_set(ATL_Uint8* msa)
{
    return (msa[0] & 0x80)?ATL_TRUE:ATL_FALSE;
}
...
inline void set_more_data(ATL_Uint8* header, ATL_Uint8 more_data)
{
    header[1] &= 0xDF;
...
inline void set_qos_prot(ATL_Uint8 *msa)
{
#if 0 //def MAC_WMM
    set_eosp(msa);
#endif /* MAC_WMM */
...
inline void set_dtim_bit(ATL_Uint8 type)
{
    if(type == AID0_BIT)
...
/* to the type given as input.                                               */
inline void reset_dtim_bit(ATL_Uint8 type)
{
    if(type == AID0_BIT)
...
inline ATL_Uint8 get_order_bit(ATL_Uint8 *header)
{
...
inline ATL_Bool is_ht_frame(ATL_Uint8 *header)
{
    if((ATL_TRUE == is_qos_bit_set(header)) && (1 == get_order_bit(header)))
        return ATL_TRUE;
...
inline void get_DA(ATL_Uint8* data, ATL_Uint8* da)
{
    if(get_to_ds(data) == 1)
...
inline ATL_Uint8 *get_DA_ptr(ATL_Uint8* data)
{
    if(get_to_ds(data) == 1)
...
inline ATL_Uint8 get_mac_hdr_len(ATL_Uint8 *msa)
{
    ATL_Uint8 mac_hdr_len = MAC_HDR_LEN;

    /* The MAC Header len is 26 only when in QOD Data frames */
    if((is_qos_bit_set(msa) == ATL_TRUE) && (get_type(msa) == DATA_BASICTYPE))
        mac_hdr_len += QOS_CTRL_FIELD_LEN;


    if(ATL_TRUE == is_ht_frame(msa))
...
/* returns ATL_TRUE if TIM needs to be reset                                 */
inline ATL_Bool update_ps_counts_txcomp(ATWILC_ieee80211_sta *ae, ATL_Uint8 *msa)
{
    ATL_Bool retval = ATL_FALSE;

    /* TBD: Device a method avoid all this processing in every Tx comp    */
    /* interrupt                                                          */
#if 0//def MAC_WMM
    BOOL_T del_ena = ATL_FALSE;
    if(ATL_TRUE == is_qos_bit_set(msa))
    {
        ATL_Uint8 priority = get_tid_value(msa);

        if(ATL_TRUE == check_ac_is_del_en(ae, priority))
            del_ena = ATL_TRUE;
    }
    ...
    if(ATL_TRUE == del_ena)
    {
        ...
        if(ae->USP_in_progress == ATL_TRUE)
        {
            ae->num_USP_pkts_qed--;
            if(ae->num_USP_pkts_qed == 0)
                ae->USP_in_progress = ATL_FALSE;
        }
    }
    else
#endif /* MAC_WMM */
...
inline ATL_Uint16 modify_frame_length( ATL_Uint8 *msa, ATL_Uint16 *rx_len, ATL_Uint16 *data_len)
{ 
    ATL_Uint32 mac_hdr_len = get_mac_hdr_len(msa);
...
inline void flush_ps_queues(ATWILC_ieee80211_sta *ae)
{
...
inline void handle_ps_sta_change_to_active(ATWILC_ieee80211_sta *ae)
{
...
/*
    ...
    {
        while((status=requeue_ps_packet(ae, &(ae->ps_q_lgcy), ATL_TRUE, ATL_FALSE))
            == PKT_REQUEUED);
        ...
        if(status == PKT_NOT_REQUEUED)
        {
            handle_requeue_pending_packet(ae, &g_ps_pending_q, ATL_TRUE);
        }
#endif /* PS_DSCR_JIT */
    }
    ...
    if(g_num_sta_ps == 0)
    {
        while(requeue_ps_packet(NULL, &g_mc_q, ATL_TRUE, ATL_FALSE) == PKT_REQUEUED);
    }
}
...
inline void handle_ps_sta_change_to_powersave(ATWILC_ieee80211_sta *ae)
{
	ATL_Bool ps_del_en_ac = ATL_FALSE;
    ...
    //update_ps_flags_ap(ae, ATL_FALSE, num_elements_moved,ps_del_en_ac);
...
*  @version		1.0
*/
void check_ps_state(ATWILC_ieee80211_sta* ae, STA_PS_STATE_T ps)
{
...
*  @version		1.0
*/
ATL_Bool filter_wlan_rx_frame(wlan_rx_t *strWlan_rx)
{
    ATWILC_ieee80211_sta *ae = (ATWILC_ieee80211_sta *)strWlan_rx->u8sa_entry;
    ATL_Uint8 *msa      = strWlan_rx->u8msa;
...
void ATWILC_Process_rx_frame(ATL_Uint8 *u8Buff, ATL_Uint32 u32Size)
{
	...
	ATL_Uint32      min_pkt_len = MAC_HDR_LEN ; //null frame
	ATL_Uint8 * mac_addr = g_linux_wlan->atwilc_netdev->dev_addr;
	...
	if(check_from_to_ds(&strWlan_rx) == ATL_TRUE) 
	{
	...
	if(check_bssid_match(&strWlan_rx) == ATL_FALSE) 
	{
	   	 if(strWlan_rx.bIs_grp_addr == ATL_FALSE)
	...
	if((strWlan_rx.bIs_grp_addr == ATL_FALSE) &&
	 (mac_addr_cmp(strWlan_rx.u8addr1,mac_addr) == ATL_FALSE) ) 
	 {
		  ATL_Uint8 *addr1 = strWlan_rx.u8addr1;
	...
	if(filter_wlan_rx_frame(&strWlan_rx) == ATL_TRUE) 
	{
		  //g_mac_stats.pewrxft++;
		  return;
	}
...
#ifdef USE_WIRELESS
	    ATWILC_WFI_monitor_rx(strWlan_rx.u8msa,strWlan_rx.u16rx_len);
#endif
  ...
  /*
	...
	// TODO: update statistics
	    /* Update receive MIB counters */
	   /* if(wlan_rx.is_grp_addr == ATL_TRUE) {
...
void ap_enabled_rx_data(ATL_Uint8* msg)
{
    //ATL_Uint8             priority_rx   = 0;
    STA_PS_STATE_T     ps            = ACTIVE_PS;
    ATL_Uint8             data_trailer  = 0;
    ATL_Uint8*		 buffer = 0 ;
    ATL_Uint8*		host_hdr =0;
    wlan_rx_t          *wlan_rx      = (wlan_rx_t *)msg;
    ATL_Uint8             *msa          = wlan_rx->u8msa;
    ATWILC_ieee80211_sta       *da_ae        = 0;
    ATWILC_ieee80211_sta       *sa_ae        = 0;
   ...
   // msdu_desc_t        *frame_desc   = 0;
    ...
    sa_ae = (ATWILC_ieee80211_sta *)wlan_rx->u8sa_entry;
    ...
    if((frm_type == NULL_FRAME) ||
       (is_sub_type_null_prot(frm_type) == ATL_TRUE))
    {
...
//#endif /* DEBUG_MODE */
   ...
   check_ps_state(sa_ae, ps);
    ...
    /* Create the MSDU descriptors for the received frame */
        ...
        sa_ae  = (ATWILC_ieee80211_sta* )find_entry(wlan_rx->u8sa);
        da_ae  = (ATWILC_ieee80211_sta* )find_entry(wlan_rx->u8da);

        if(da_ae != 0)
        {
            //ATL_Uint8 key_type        = 0;
            //ATL_Bool wlan2wlan       = ATL_FALSE;
            ATL_Bool use_same_buffer = ATL_TRUE;
            ...
            if(da_ae->state == ASOC)
            {
				...
				/* header.                                               */
				//if(is_qos_bit_set(msa) == ATL_FALSE)
					//use_same_buffer = ATL_FALSE;
		...
		// send this packet to wlan interface
			...
			tx_data->skb  = NULL;
                ATWILC_Xmit_data((void*)tx_data, WLAN_TO_WLAN);
        ...
        else
        {
            ...
            if(is_group(wlan_rx->u8da) == ATL_TRUE)
            {
               ...
               // if(ap_check_sec_tx_sta_state(sa_ae) != ATL_TRUE)
                //{
                  //  continue;
                //}
...
//#endif /* DEBUG_MODE */
            ...
            if(ATL_TRUE == is_snap_header_present(wlan_rx))
            {
		...
		ATL_memcpy(host_hdr, wlan_rx->u8da, ETH_ALEN);
        	ATL_memcpy(host_hdr+6, wlan_rx->u8sa, ETH_ALEN);
...
int ATWILC_Xmit_data(void* ptx_data, Data_Interface InterfaceType)
{
	...
	struct ethhdr 			*eth;
	ATWILC_ieee80211_sta	*asoc_entry = NULL;
	ATL_Uint8 			ethsaddr[ETH_ALEN];
	ATL_Uint8			ethdaddr[ETH_ALEN];
	ATL_Uint8			mac_hdr_len;
	ATL_Uint8*			da;
	ATL_Uint8	 		service_class, is_qos,is_htc, is_amsdu;
	ATL_Uint8			priority = BEST_EFFORT_PRIORITY; // TODO: this should not be fixed
	ATL_Uint8* 			BSSID = g_linux_wlan->atwilc_netdev->dev_addr;
	ATL_Uint8* 			snap_hdr;
	ATL_Uint8* 			mac_hdr;
	ATL_Uint8* 			data_start;
	ATL_Uint16 			eth_type;
	ATL_Uint16 			pkt_len = 0;
	ATL_Uint16 			data_len =0;
	ATL_Uint16			u16size=0;
	...
	if (InterfaceType == HOST_TO_WLAN)
	{
		...
		ATL_memcpy(ethdaddr, &eth->h_dest, ETH_ALEN);

	    	/* For AP, the source address should be sent with the frame. */
		ATL_memcpy(ethsaddr, &eth->h_source, ETH_ALEN);
		...
		/* For unicast packets, check if the destination STA is associated. */
	    	if(is_group(ethdaddr) == ATL_FALSE)
	    	{
		        /* Get association entry for association state */
		        asoc_entry = (ATWILC_ieee80211_sta*)find_entry(ethdaddr);
	...
	else if(InterfaceType == MONITOR_TO_WLAN)
	{
		...
		/* Get association entry for association state */
		asoc_entry = (ATWILC_ieee80211_sta*)find_entry(da);

		
		if(buffer_tx_packet(asoc_entry, da, priority , tx_data) == ATL_FALSE)
	    	{
			atwilc_wlan_txq_add_mgmt_pkt((void*)tx_data,tx_data->buff,tx_data->size,linux_FH_wlan_tx_complete);
		}
	...
	if (InterfaceType == HOST_TO_WLAN)
	{
	       ...
	       (eth_type == LLTD_TYPE))
	    {
	        ...
	        *snap_hdr++ = 0x00;
	        *snap_hdr++ = 0x00;
		 *snap_hdr++ = (ATL_Uint8)((eth_type>>8) &0x00ff);
		 *snap_hdr++ = (ATL_Uint8)(eth_type &0x00ff);
		...
		ATL_memcpy(data_start, (skb->data) + ETHERNET_HDR_LEN , pkt_len - ETHERNET_HDR_LEN);
	...
	printk("data_len = %d\n",tx_data->size);
	for(i = 0 ; i < tx_data->size ; i++)
		printk("msa2[%d] = %x\n",i,((ATL_Uint8*)(tx_data->buff))[i]);
	}	*/
	
	/*QueueCount = atwilc_wlan_txq_add_mgmt_pkt((void*)tx_data,
									tx_data->buff,
									tx_data->size,
									linux_FH_wlan_tx_complete);*/

	//QueueCount = atwilc_FH_wlan_txq_add_net_pkt(tx_data,tx_data->buff,tx_data->size,linux_FH_wlan_tx_complete);

	
		  
	if(buffer_tx_packet(asoc_entry, da, priority , tx_data) == ATL_FALSE)
    	{
		QueueCount = atwilc_FH_wlan_txq_add_net_pkt((void*)tx_data,
										tx_data->buff,
										tx_data->size,
										linux_FH_wlan_tx_complete);
	}
...
*  @version		1.0
*/
ATL_Uint32 hash(ATL_Uint8* addr)
{
    ATL_Uint32 sum = 0;
    ATL_Uint32  i   = 0;	
...
void* find_entry(ATL_Uint8* key)
{
    ...
    while(tbl_ptr)
    {
        if(mac_addr_cmp(tbl_ptr->key, key) == ATL_TRUE)
...
void add_entry(void* entry, ATL_Uint8* key)
{
    ...
    ATL_memcpy(new_elm->key, key, ETH_ALEN);
...
void handle_ps_tx_comp_ap(struct tx_complete_data *tx_dscr )
{
    ATWILC_ieee80211_sta *ae;
    ATL_Uint8 *msa  = tx_dscr->buff;
    ATL_Uint8  *da   = NULL;

    /* Get the Destination address */
    da = get_DA_ptr(msa);

    ae = (ATWILC_ieee80211_sta*)find_entry(da);
	
    if(is_group(da) ==  ATL_TRUE)
    {
    ...
    else
    {
        ...
        {
            if(ATL_TRUE == update_ps_counts_txcomp(ae, msa))
            {
                check_and_reset_tim_bit(ae->u16AID);
            }
        }
...
void check_and_reset_tim_bit(ATL_Uint16 asoc_id)
{
    ATL_Uint8  byte_offset    = 0;
    ATL_Uint8  bit_offset     = 0;
    ATL_Uint8  pvb_offset     = 0;
    ATL_Uint8  length         = 0;
    ATL_Uint8  new_pvb_offset = 0;
    int16_t	 	i              = 0;
    ATL_Uint8  new_length     = MIN_TIM_LEN;
                                   ...
                                   ATL_Bool ps_q_legacy, ATL_Bool eosp)
{
    ...
    ATL_Uint8   srv_cls      = 0;
    ATL_Uint8   tid          = 0;
    ATL_Uint32  phy_tx_mode  = 0;
    ATL_Bool   is_qos       = ATL_FALSE;
    ATL_Uint32  retry_set[2] = {0};
    ...
    /* If the queue has no packets return ATL_FALSE to indicate that no packets */
	...
	//if((is_qos == ATL_TRUE) && ( !((get_sub_type(mac_hdr) == ACTION ) && (mac_hdr[MAC_HDR_LEN+CATEGORY_OFFSET] == BA_CATEGORY))))
    ...
    /* is returned                                                           */
    if(ps_q_legacy == ATL_FALSE)
    {
        if(is_end_prot(ae) == ATL_TRUE)
        {
            set_qos_prot(mac_hdr);
            end_of_q = ATL_TRUE;
        }
    ...
    if(ps_q_legacy == ATL_TRUE)
    {
	    if(is_group(addr1) == ATL_TRUE)
	    {
	        g_num_mc_bc_pkt--;
	    }
	    else
    ...
    //if(ATL_FALSE == is_serv_cls_buff_pkt((ATL_Uint8 *)ae, q_num, tid, tx_dscr))
    {	

	// TODO: add EAPOL and other monitor types check here
	if(get_sub_type(tx_dscr->buff) == NULL_FRAME)
	{
		atwilc_wlan_txq_add_mgmt_pkt((void*)tx_dscr,
									 tx_dscr->buff,
									 tx_dscr->size,
									 linux_FH_wlan_tx_complete);
	}
	...
	{
	    	 atwilc_FH_wlan_txq_add_net_pkt((void*)tx_dscr,
										tx_dscr->buff,
										tx_dscr->size,
										linux_FH_wlan_tx_complete);
	}
...
//#ifdef DEBUG_MODE
        //    g_mac_stats.qaexc++;
//#endif /* DEBUG_MODE */
           // free_tx_dscr((ATL_Uint32 *)tx_dscr);
...
*  @version		1.0
*/	
void set_tim_bit(ATL_Uint16 asoc_id)
{
    ATL_Uint8 byte_offset = 0;
    ATL_Uint8 bit_offset  = 0;
    ATL_Uint8 pvb_offset  = 0;
...
*  @version		1.0
*/	
void update_ps_flags_ap(ATWILC_ieee80211_sta *ae, ATL_Bool bc_mc_pkt,
                        ATL_Uint8 num_buff_added, ATL_Bool ps_add_del_ac)
{
    if(num_buff_added == 0)
        return;

    if(bc_mc_pkt == ATL_TRUE)
    {
...
/*                                                                           */
/*  Returns       : ATL_TRUE, if the packet should be buffered for power save   */
/*                  ATL_FALSE, if the packet should not be buffered             */
/*                                                                           */
/*  Issues        : None                                                     */
/*                                                                           */
/*  Revision History:                                                        */
...
ATL_Bool is_ps_buff_pkt_ap(ATWILC_ieee80211_sta *ae, ATL_Uint8 *da, struct tx_complete_data* dscr)
{
    ATL_Bool  ps_del_en_ac    = ATL_FALSE;
    ATL_Bool  bc_mc_pkt       = is_group(da);
    ATL_Uint8  priority        = 0;
    list_buff_t *qh         = NULL;
    ...
    if( qh != NULL)
    {
        ...
        update_ps_flags_ap(ae, bc_mc_pkt, 1, ATL_FALSE);
    }
    else
    {
        if(dscr->skb)
...
/*  Outputs       : None                                                     */
/*                                                                           */
/*  Returns       : ATL_TRUE, if the given packet is buffered in any queue      */
/*                  ATL_FALSE, if the packet is not buffered and can be sent    */
/*                                                                           */
/*  Issues        : None                                                     */
/*                                                                           */
/*  Revision History:                                                        */
...
ATL_Bool buffer_tx_packet(ATL_Uint8 *entry, ATL_Uint8 *da, ATL_Uint8 priority,
                         struct tx_complete_data *tx_dscr)
{
    if(ATL_TRUE == is_ps_buff_pkt_ap((ATWILC_ieee80211_sta*)entry, da, tx_dscr))
    {
#ifdef DEBUG_MODE
        g_mac_stats.psbuff++;
#endif /* DEBUG_MODE */
    ...
    else if(ATL_TRUE == is_serv_cls_buff_pkt(entry, priority, tx_dscr))
    {
#ifdef DEBUG_MODE
    ...
    /* If the packet is buffered return ATL_TRUE */
    return ATL_TRUE;
}
...
void tx_null_frame(ATL_Uint8 *sa, ATWILC_ieee80211_sta *ae, ATL_Bool is_qos, ATL_Uint8 priority,ATL_Uint8 more_data)
{
    ATL_Uint8  q_num          = 0;
    ATL_Uint8  tx_rate        = 0;
    ATL_Uint8  pream          = 0;
   ...
   tx_data = (struct tx_complete_data*)kmalloc(sizeof(struct tx_complete_data),GFP_ATOMIC);
	...
	len = MAC_HDR_LEN ;//+ FCS_LEN;
       ...
       msa = (ATL_Uint8*)kmalloc(len,GFP_ATOMIC);
	...
	set_frame_control(msa, (ATL_Uint16)NULL_FRAME);
    ...
    set_address1(msa, sa);
    set_address2(msa, g_linux_wlan->atwilc_netdev->dev_addr);
    set_address3(msa, g_linux_wlan->atwilc_netdev->dev_addr);
...
ATL_Bool handle_ps_poll(wlan_rx_t    *wlan_rx )
{
    ...
    ATWILC_ieee80211_sta *ae       = (ATWILC_ieee80211_sta*)(wlan_rx->u8sa_entry);
    ...
    if(ae->ps_poll_rsp_qed == ATL_TRUE)
    {
    ...
    if(peek_list(&(ae->ps_q_lgcy)) == NULL)
        tx_null_frame(wlan_rx->u8sa, ae, ATL_FALSE, 0, 0);
    else
        requeue_ps_packet(ae, &(ae->ps_q_lgcy), ATL_TRUE, ATL_FALSE) ;

    ae->ps_poll_rsp_qed = ATL_TRUE;
...
ATL_Bool filter_monitor_data_frames(ATL_Uint8 *buf, ATL_Uint16 len)
{
	...
	if(get_type(buf)== DATA_BASICTYPE)
	{
		...
		tx_data->size = len;
		tx_data->skb  = NULL;
                ATWILC_Xmit_data((void*)tx_data, MONITOR_TO_WLAN);
