...
void wilc_debug(uint32_t flag, char *fmt, ...)
{
	char buf[256];
	va_list args;
	int len;
...
CHIP_PS_STATE_T genuChipPSstateFromWifi = CHIP_WAKEDUP;
...
void chip_sleep_manually(WILC_Uint32 u32SleepTime)
{
	...
	WILC_PRINTF("in chip_sleep_manually()\n");
		...
		**/
		...
		do {
			...
			**/
			...
			if (!ret) {
				wilc_debug(N_ERR, "[wilc txq]: fail can't write reg 0x10a8..\n");
				break;
			}
			...
			**/
			...
			do {
				...
				if (!ret) {
					wilc_debug(N_ERR, "[wilc txq]: fail can't read reg 0x10a8..\n");
					break;
				}
				if (reg == 0) {
					/**
...
static void wilc_pllupdate_isr_ext(uint32_t int_stats){
	...
	while(!(ISWILC3000(wilc_get_chipid(WILC_TRUE))&&--trials)) {
...
#endif
#if 0 // EXT_PA, XTAL_24, DISABLE_WILC_UART not supported for now for WILC3000
...
/*Enable/Disable GPIO configuration for FW logs*/
	...
	**/
	...
	if (!ret) {
		wilc_debug(N_ERR, "[wilc start]: fail read reg 0x3b0000 ...\n");
		release_bus(RELEASE_ONLY, PWR_DEV_SRC_WIFI);
		/* EIO  5*/
		ret = -5;
 		return ret;
...
uint32_t init_chip(void)
{
	...
	printk("Bootrom sts = %x\n",reg);
		...
		**/
		...
		if (!ret) {
			wilc_debug(N_ERR, "[wilc start]: fail read reg 0x3b0090 ...\n");
			return ret;
		}
		reg |= (1 << 0);
		ret = g_wlan.hif_func.hif_write_reg(0x3b0090, reg);
		if (!ret) {
			wilc_debug(N_ERR, "[wilc start]: fail write reg 0x3b0090 ...\n");
			return ret;
		}
#endif
		/**
		...
		**/
		...
		if (!ret) {
			wilc_debug(N_ERR, "[wilc start]: fail write reg 0x4f0000 ...\n");
			return ret;
